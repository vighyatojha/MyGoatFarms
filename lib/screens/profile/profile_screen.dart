...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _showAddPartnerSheet() async {
    ...
    // Uses the shared AddPartnerSheet (lib/widgets/add_partner_sheet.dart)
    // rather than a local copy. This screen used to have its own
    // `_AddPartnerSheet` that called FirestoreService.createPartner()
    // directly — a second, divergent "add partner" path that skipped the
    // approval workflow entirely (no `status`, no `permissions`), so a
    // partner added from here got full Firestore access via
    // isOwnerOrPartner() while every PartnerAccessService.can() check
    // silently failed (empty permissions), and PartnerManagementScreen
    // had nothing to approve. Routing through the same sheet everywhere
    // means there's exactly one way a partner gets created, and it's
    // always the pending-approval one.
    await showModalBottomSheet<bool>(
      ...
      builder: (_) {
        return AddPartnerSheet(
          farmId: _farmId!,
        );
      },
    );

    // AddPartnerSheet shows its own confirmation SnackBar on success, and
    // the new partner shows up via _partnerSub's stream, so there's
