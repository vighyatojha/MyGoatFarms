...
class _AddFeedStockScreenState extends State<AddFeedStockScreen> {
  ...
  // When on, this purchase is NOT paid in cash — it's tracked as an
  // amount owed to a chosen supplier via SupplierLedger, instead of
  // using the free-text _supplierController / Payment Method chips
  // above (those stay for cash purchases).
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (totalCost > 0) {
        ...
        // Buy on Credit: this purchase wasn't paid for, so track it as
        // an amount now owed to the chosen supplier — same ledger the
        // Supplier Ledger screen (Finance tab) reads from.
        if (_isCredit) {
      ...
      _message(
        _isCredit
            ? 'Feed stock added — ₹${totalCost.toStringAsFixed(0)} added to ${_selectedSupplier!.name}\'s credit'
            : totalCost > 0
  ...
  /// quickly add a new one — mirrors the existing "add new feed" quick
  /// dialog pattern used by [_promptNewFeedName].
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        const Icon(Icons.add_rounded, size: 15, color: AppColors.primaryGreen),
                        const SizedBox(width: 4),
                        Text(
                          'New Supplier',
                          style: AppTheme.body(size: 12, color: AppColors.primaryGreen, weight: FontWeight.w700),
                        ),
                      ],
                    ),
                  ),
  ...
  /// Quick-add dialog for a new supplier, mirroring [_promptNewFeedName]'s
  /// dialog pattern and controller-disposal note.
  Future<void> _promptNewSupplier(String farmId) async {
    ...
    // See the identical note on _promptNewFeedName: controller is not
    // disposed here on purpose, to avoid racing the dialog's exit
    // transition.
