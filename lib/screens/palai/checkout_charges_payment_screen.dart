...
// ================================================================
  ...
  Future<void> _completeCheckout() async {
    ...
    try {
// ------------------------------------------------------------
// RESERVE (DO NOT CREATE) THE BILL.
//
// reserveBillId() only allocates a local document reference —
// it never touches Firestore. The Final Checkout Report PDF is
// generated using this reserved number so it's correct from
// the start, but nothing is actually written until the user
// presses Done inside that screen (see onDone below). This is
// what makes "Generate/Download/Share happens before anything
// is committed" actually true, instead of just true for the
// goat-checkout half of it.
// ------------------------------------------------------------

      final reserved = FirestoreService.instance.reserveBillId(widget.farmId);

      final previewBillResult = MonthlyBillResult(
        billId: reserved.billId,
        billNumber: reserved.billNumber,
        previousPending: _previousPending,
        advanceBefore: _advanceBefore,
        newCharges: _newCharges,
        advanceApplied: _advanceApplied,
        totalDue: _totalDue,
        paid: _paid,
        amountAppliedToBill: _paid.clamp(0, _totalDue).toDouble(),
        pendingAfter: _pendingAfter,
        advanceAfter: _advanceAfter,
...
//        ↓
// ONLY THEN: create the bill AND mark goats as checked out
//
// Therefore NOTHING is written to Firestore here — not the
// bill, not the goat checkout. Both happen together, atomically
// from the user's point of view, inside onDone below.
// ------------------------------------------------------------
              ...
              FinalCheckoutReportScreen(
                ...
                onDone: () async {
                  await FirestoreService.instance.createMonthlyBill(
                    ...
                    reservedBillId: reserved.billId,
                    reservedBillNumber: reserved.billNumber,
                  );
